...
from services.state.session_default import *
from services.config.workout_config import *
from services .ui.style_ladder import *
...
def main():
    ...
    with st.sidebar:
        ...
        if not workout_started:
            ...
            if start_session_button:
                 ...
                 st.session_state.sets_cycle_started_at = time.time()
    ...
    if isinstance(user_id,int):
        ...
        if not df.empty:
             ...
             agg_df["Workout Time"] = agg_df["Time (sec)"].apply(format_time)
             agg_df.drop(columns=["Time (sec)"], inplace=True)
             agg_df.index +=1
